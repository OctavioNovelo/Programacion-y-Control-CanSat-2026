...
/* USER CODE BEGIN PV */
volatile uint8_t tx_ready = 1; // 1 = El radio está libre para enviar
uint32_t last_tick = 0;
CanSat_Packet telemetria;

// --- VARIABLES DE DEBUGGING ---
volatile uint8_t sx1278_version = 0;
volatile uint8_t sx1278_detected = 0;
volatile int g_sh2_open_rc = -1;
volatile uint8_t init_bme = 0;
...
int main(void)
{
    ...
    sx1278_version = sx1278_read_reg(0x42);
    if (sx1278_version == 0x12)
    {
        sx1278_detected = 1;
    }
    ...
    while (1)
    {
        ...
        sensors_service();
        // Lógica de disparo cada 500ms
        if (HAL_GetTick() - last_tick >= 500 && tx_ready)
        {

            last_tick = HAL_GetTick();
            // 1. Leer sensores (BME/BNO)
            read_sensors(&telemetria);
            // 2. Bloquear radio y enviar
            tx_ready = 0;
            sx1278_transmit_it((uint8_t *)&telemetria, sizeof(telemetria));
            tx_count++;
        }
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
    ...
    {
        // Aquí notificaremos al driver del BNO que hay datos listos
